...
def knn_after_unknown_factor():
    """
    Train and evaluate KNN model after including the unknown factor, with detailed metrics.
    """
    from sklearn.neighbors import KNeighborsClassifier
    from sklearn.model_selection import train_test_split
    from sklearn.metrics import (
    ...
    # Assuming the unknown factor has been processed and added to the dataset
    # Use the full dataset, including 'Maybe' values and any additional features
    X = health_data.drop(['Growing_Stress', 'Timestamp', 'Country', 'Occupation'], axis=1, errors='ignore')
    ...
    # Print Results
    print("\nKNN Results (After Unknown Factor):")
    print(f"Accuracy: {accuracy:.4f}")
    print(f"Precision: {precision:.4f}")
    print(f"Recall: {recall:.4f}")
    print(f"F1-Score: {f1:.4f}")
    ...
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title("ROC Curve (After Unknown Factor)")
    plt.legend(loc="lower right")
    plt.show()

# Call the functions to train and evaluate KNN models
knn_before_unknown_factor()
knn_after_unknown_factor()
