...
def cross_validation_logistic_regression():
    """
    Perform cross-validation for logistic regression model after removing 'Maybe' from Growing_Stress.
    """
    import pandas as pd
    from sklearn.linear_model import LogisticRegression
    from sklearn.model_selection import cross_val_score
    ...
    cross_val_auc = cross_val_score(logit_model, X_filtered, y_filtered, cv=6, scoring='roc_auc')
    cross_val_acc = cross_val_score(logit_model, X_filtered, y_filtered, cv=5, scoring='accuracy')

    # Print Cross-Validation Results
    print("\nCross-Validation Results:")
    print(f"Mean ROC-AUC (5-Fold): {np.mean(cross_val_auc):.4f}")
    print(f"Mean Accuracy (5-Fold): {np.mean(cross_val_acc):.4f}")
